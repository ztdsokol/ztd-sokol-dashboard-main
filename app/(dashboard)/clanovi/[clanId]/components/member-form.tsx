...
export const MemberForm: React.FC<MemberFormProps> = ({ initialData }) => {
  ...
  const description = initialData
    ...
    : "Dodaj novog člana";
  const toastMessage = initialData ? "Član updated." : "Člana created.";
  ...
  const onSubmit = async (data: MemberFormValues) => {
    try {
      setLoading(true);
      if (initialData) {
        await axios.patch(`/api/clanovi/${params.clanId}`, data);
      } else {
        await axios.post(`/api/clanovi`, data);
      }
      router.refresh();
      router.push(`${origin}/clanovi`);
  ...
  const onDelete = async () => {
    try {
      setLoading(true);
      await axios.delete(`/api/clanovi/${params.clanId}`);
      router.refresh();
      router.push("/clanovi");
      toast.success("člana je obrisan.");
    } catch (error: any) {
